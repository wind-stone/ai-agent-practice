...
`)
    ...
    for (let i = 0; i < maxIterations; i++) {
        console.log(chalk.bgGreen(`⏳ 正在等待 AI 思考...第 ${i} 轮思考`));
        ...
        for await (const chunk of rawStream) {
            ...
            //
            // 疑问：为什么可以用 concat 组成完整的 fullAIMessage
            //
            // LangChain 为 AIMessageChunk 实现了特殊的 concat（拼接）方法。这个方法并不是简单的字符串相加，而是带有智能合并逻辑的：
            // - 文本累加：它会自动将新 chunk 的 content 拼接到旧消息的 content 后面。
            // - 参数合并：如果遇到 tool_call_chunks，它会识别出这些碎片属于同一个工具调用，并将它们按顺序拼凑成一段越来越长的 JSON 字符串。
            // - 保持结构：除了增量部分，它还会保留消息的 id 等基础信息不变。
        ...
        // 检查是否有工具调用
        if (
            !fullAIMessage.tool_calls ||
            fullAIMessage.tool_calls.length === 0
        ) {
