...
const EPUB_FILE =
    '/Users/WindStone/github/ai-agent-practice/apps/milvus-test/src/assets/天龙八部.epub';
...
async function loadAndProcessEPubStreaming(bookId: number) {
    try {
        ...
        for (
            ...
            console.log(
                `即将处理第 ${chapterIndex + 1}/${documents.length} 章...`
            );
        ...
        console.error('加载 EPUB 文件时出错:', (error as Error).message);
        throw error;
    }
}
