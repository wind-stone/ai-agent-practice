 ...
 */
async function retrieveRelevantContent(question: string, k = 3) {
    try {
        // 生成问题的向量
        ...
        console.error('检索内容时出错:', (error as Error).message);
        return [];
    }
}
...
`; // 5. 调用 LLM 生成回答
        ...
        console.error('回答问题时出错:', (error as Error).message);
...
async function main() {
    try {
        ...
        try {
            ...
            // 如果已经加载，会报错，忽略即可
            if (!(error as Error).message.includes('already loaded')) {
                throw error;
            }
