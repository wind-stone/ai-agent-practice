...
interface IObjectWithNum {
    num: number;
}

const addOne = RunnableLambda.from((input: IObjectWithNum) => input.num + 1);

const multiplyTwo = RunnableLambda.from(
    (input: IObjectWithNum) => input.num * 2
);

const square = RunnableLambda.from(
    (input: IObjectWithNum) => input.num * input.num
);
